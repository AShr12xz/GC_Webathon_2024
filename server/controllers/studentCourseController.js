...
exports.showAttendanceforStudent = (async (req, res) => {
    ...
    const { courseName, coursecode, attended, classes } = student;

    res.status(201).json({
        status: "success",
        data: {
            courseName, coursecode, attended: attended.length, classes: classes.length
        },
    })
...
exports.showAttendanceforFaculty = (async (req, res) => {
    ...
    const faculty = await studentCourse.find({ facultycode: req.body.facultycode });
...
exports.updateAttendance = asyncCheck(async (req, res) => {
    ...
    req.body.map(async (el) => {
        console.log(el);
        const student = await studentCourse.findOne({ rollno: el.rollno, coursecode: el.coursecode });
        console.log(student);
        if(el.attended !== '0')
        {
