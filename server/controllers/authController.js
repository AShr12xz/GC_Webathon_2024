...
exports.login = asyncCheck(async (req, res, next) => {
  ...
  if (user.role === 'faculty') {
    const facultyToken = signToken(user._id, 'faculty'); // Generate a faculty-specific token
    res.status(200).json({
      status: 'success',
      token: facultyToken, // Send faculty token
      data: {
        user: user
      }
    });
  } else {
    // Generate a regular token for non-faculty users
